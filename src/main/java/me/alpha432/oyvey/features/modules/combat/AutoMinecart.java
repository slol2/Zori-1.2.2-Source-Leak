...
public class AutoMinecart extends Module {
    private final Setting<Boolean> web = register(new Setting<Boolean>("Web", Boolean.FALSE));
    private final Setting<Boolean> rotate = register(new Setting<Boolean>("Rotate", Boolean.FALSE));
    private final Setting<Boolean> packet = register(new Setting<Boolean>("PacketPlace", Boolean.FALSE));
    private final Setting<Integer> blocksPerTick = register(new Setting<Integer>("BlocksPerTick", 1, 1, 4));
    private final Setting<Integer> delay = register(new Setting<Integer>("Carts", 20, 0, 50));
    public Setting<Float> minHP = register(new Setting<Float>("MinHP", Float.valueOf(4.0f), Float.valueOf(0.0f), Float.valueOf(36.0f)));
    int wait;
    int waitFlint;
    int originalSlot;
    ...
    private EntityPlayer getTarget() {
        ...
        for (EntityPlayer player : AutoMinecart.mc.world.playerEntities) {
            if (EntityUtil.isntValid(player, 6.0) || player.isInWater() || player.isInLava() || !EntityUtil.isTrapped(player, false, false, false, false, false) || player.getHealth() + player.getAbsorptionAmount() > minHP.getValue().floatValue())
                continue;
            if (target == null) {
                target = player;
